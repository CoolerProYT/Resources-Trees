...
public class TreeSimulatorCategory extends AbstractRecipeCategory<RecipeEntry<TreeSimulatorRecipe>> {
    ...
    public static final IRecipeHolderType<TreeSimulatorRecipe> TREE_SIMULATOR_TYPE = IRecipeHolderType.create(ModRecipes.TREE_SIMULATOR_TYPE);
    private int tickCount = 0;

    public TreeSimulatorCategory(IGuiHelper helper) {
        super(TREE_SIMULATOR_TYPE, Text.translatable("block.resourcestrees.tree_simulator"), helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ModBlocks.TREE_SIMULATOR)), 168, 77);
    }

    @Override
    public void draw(RecipeEntry<TreeSimulatorRecipe> recipe, IRecipeSlotsView recipeSlotsView, DrawContext guiGraphics, double mouseX, double mouseY) {
        guiGraphics.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, 0, 0, 5, 5, 168, 77, 256, 256);

        tickCount++;
        int arrowWidth = (tickCount % 600) * 23 / 600;

        Identifier texture = ResourcesTrees.id("textures/gui/progress.png");
        guiGraphics.drawTexture(RenderPipelines.GUI_TEXTURED, texture, 54, 30, 0, 0, arrowWidth, 16, 22, 16);

        guiGraphics.fill(56, 53, 75, 72, 0xFFC6C6C6);

        Text text = Text.translatable("tooltip.resourcestrees.tickToGrow", recipe.value().ticksToGrow());
        guiGraphics.drawText(MinecraftClient.getInstance().textRenderer, text, 0, 70, 0xFF666666, false);
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder iRecipeLayoutBuilder, RecipeEntry<TreeSimulatorRecipe> recipeHolder, IFocusGroup iFocusGroup) {
