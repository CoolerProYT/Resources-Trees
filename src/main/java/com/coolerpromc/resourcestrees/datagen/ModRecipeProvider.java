...
public class ModRecipeProvider extends FabricRecipeProvider {
    public static final Map<ResourcesSaplingBlock, Item> SAPLINGS_BY_SAPLINGS = Map.of(
            ...
            ModBlocks.RESOURCES_PALE_OAK_SAPLING,  Items.PALE_OAK_SAPLING
    );

    private RegistryEntryLookup<Item> items;
    private RegistryEntryLookup<ResourcesTypes> types;
    ...
    @Override
    protected RecipeGenerator getRecipeGenerator(RegistryWrapper.WrapperLookup wrapperLookup, RecipeExporter output) {
        this.items = wrapperLookup.getOrThrow(RegistryKeys.ITEM);
        this.types = wrapperLookup.getOrThrow(ModRegistries.RESOURCES_TYPES_KEY);
        this.output = output;
        
        return new RecipeGenerator(wrapperLookup, output) {
            @Override
            public void generate() {
                ...
                // Block Recipe
                ShapedRecipeJsonBuilder.create(items, RecipeCategory.BUILDING_BLOCKS, ModBlocks.TREE_SIMULATOR, 1)
                        .pattern("   ")
                        .pattern("ABA")
                        .pattern("AAA")
                        ...
                        try {
                            ...
                            if (obj instanceof ResourcesSaplingBlock resourcesSaplingBlock){
                                ...
                                if (value.material().left().isPresent()){
                                    ExtendedShapedRecipeBuilder.create(items, RecipeCategory.BUILDING_BLOCKS, sapling)
                                            .pattern(" A ")
                                            .pattern("ABA")
                                            .pattern(" A ")
                                            ...
                                            .offerTo(output, RegistryKey.of(RegistryKeys.RECIPE, key.withSuffixedPath(Registries.BLOCK.getId(resourcesSaplingBlock).getPath().substring(9)).withPrefixedPath("saplings/")));
                                }
                                ...
                                TreeSimulatorRecipeBuilder.builder()
                                        ...
                                        .setTicksToGrow(1200)
                                        .offerTo(output, RegistryKey.of(RegistryKeys.RECIPE, key.withSuffixedPath(Registries.BLOCK.getId(resourcesSaplingBlock).getPath().substring(9)).withPrefixedPath("tree_simulator/")));
                            }
    ...
    private void essenceItem(Item ouputItem, Item... inputItems){
        ShapelessRecipeJsonBuilder builder = ShapelessRecipeJsonBuilder.create(items, RecipeCategory.MISC, ouputItem, 1);
    ...
    private void circleShape(Item outputItem, int count, RegistryKey<ResourcesTypes> resourceType){
        ShapedRecipeJsonBuilder.create(items, RecipeCategory.BUILDING_BLOCKS, outputItem, count)
                ...
                .pattern("A A")
                .pattern("AAA")
                .input('A', new ComponentsIngredient(Ingredient.ofItem(ModItems.LEAF_FRAGMENT), ComponentChanges.builder().add(ModDataComponents.TYPE, types.getOrThrow(resourceType).registryKey().getValue()).build()).toVanilla())
    ...
    private void circleSurroundedShapeWithItem(Item outputItem, int count, RegistryKey<ResourcesTypes> resourceType, Item item){
        ShapedRecipeJsonBuilder.create(items, RecipeCategory.BUILDING_BLOCKS, outputItem, count)
                ...
                .pattern("ABA")
                .pattern("AAA")
                .input('B', new ComponentsIngredient(Ingredient.ofItem(ModItems.LEAF_FRAGMENT), ComponentChanges.builder().add(ModDataComponents.TYPE, types.getOrThrow(resourceType).registryKey().getValue()).build()).toVanilla())
                .input('A', item)
                .criterion(hasItem(ModItems.LEAF_FRAGMENT, types.getOrThrow(resourceType).registryKey().getValue()), has(ModItems.LEAF_FRAGMENT, types.getOrThrow(resourceType).registryKey().getValue()))
    ...
    private void circleSurroundedShape(Item outputItem, int count, RegistryKey<ResourcesTypes> resourceType, RegistryKey<ResourcesTypes> middleResourceType){
        ShapedRecipeJsonBuilder.create(items, RecipeCategory.BUILDING_BLOCKS, outputItem, count)
                ...
                .pattern("AAA")
                .input('A', new ComponentsIngredient(Ingredient.ofItem(ModItems.LEAF_FRAGMENT), ComponentChanges.builder().add(ModDataComponents.TYPE, types.getOrThrow(resourceType).registryKey().getValue()).build()).toVanilla())
                .input('B', new ComponentsIngredient(Ingredient.ofItem(ModItems.LEAF_FRAGMENT), ComponentChanges.builder().add(ModDataComponents.TYPE, types.getOrThrow(middleResourceType).registryKey().getValue()).build()).toVanilla())
                .criterion(hasItem(ModItems.LEAF_FRAGMENT, types.getOrThrow(resourceType).registryKey().getValue()), has(ModItems.LEAF_FRAGMENT, types.getOrThrow(resourceType).registryKey().getValue()))
    ...
    private void twoItemCustomShape(Item outputItem, int count, RegistryKey<ResourcesTypes> resourceType, RegistryKey<ResourcesTypes> resourceType2, String line1, String line2, String line3){
        ShapedRecipeJsonBuilder.create(items, RecipeCategory.BUILDING_BLOCKS, outputItem, count)
                ...
                .pattern(line3)
                .input('A', new ComponentsIngredient(Ingredient.ofItem(ModItems.LEAF_FRAGMENT), ComponentChanges.builder().add(ModDataComponents.TYPE, types.getOrThrow(resourceType).registryKey().getValue()).build()).toVanilla())
                .input('B', new ComponentsIngredient(Ingredient.ofItem(ModItems.LEAF_FRAGMENT), ComponentChanges.builder().add(ModDataComponents.TYPE, types.getOrThrow(resourceType2).registryKey().getValue()).build()).toVanilla())
                .criterion(hasItem(ModItems.LEAF_FRAGMENT, types.getOrThrow(resourceType).registryKey().getValue()), has(ModItems.LEAF_FRAGMENT, types.getOrThrow(resourceType).registryKey().getValue()))
    ...
    private AdvancementCriterion<?> has(TagKey<Item> item) {
        return inventoryTrigger(ItemPredicate.Builder.create().tag(this.items, item));
    }

    protected AdvancementCriterion<?> has(ItemConvertible itemLike, Identifier key) {
        return inventoryTrigger(ItemPredicate.Builder.create().items(this.items, itemLike).components(ComponentsPredicate.Builder.create().exact(ComponentMapPredicate.builder().add(ModDataComponents.TYPE, key).build()).build()));
    }
