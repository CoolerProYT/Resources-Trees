...
public class TreeSimulatorCategory extends AbstractRecipeCategory<RecipeHolder<TreeSimulatorRecipe>> {
    ...
    @Override
    public void draw(RecipeHolder<TreeSimulatorRecipe> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.text(Minecraft.getInstance().font, text, 0, 70, 0xFF666666, false);
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder iRecipeLayoutBuilder, RecipeHolder<TreeSimulatorRecipe> recipeHolder, IFocusGroup iFocusGroup) {
        ...
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.INPUT,21,30).add(recipe.tree().create());

        List<TreeSimulatorOutput> drops = recipe.drops();
        for (int i = 0; i < drops.size(); i ++){
            int finalI = i;
            iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.OUTPUT, 93 + 18 * (i % 3), 12 + (i / 3) * 18).add(drops.get(i).output().create()).addRichTooltipCallback((r, t) -> tooltipCallback(r, t, drops.get(finalI)));
        }
    }
