...
public class FabricCompatHelper implements ICompatHelper {
    ...
    public void injectAgritechRecipe(BlockRegistryHandler<ResourcesSaplingBlock> handler, List<RecipeHolder<?>> recipeHolders, Set<ResourceKey<Recipe<?>>> existingKeys, HolderLookup.RegistryLookup<Item> items) {
        ...
        com.misterd.agritech.recipe.TreeRecipe recipe = new com.misterd.agritech.recipe.TreeRecipe(
            Ingredient.of(block),
            List.of(Ingredient.of(items.getOrThrow(TagKey.create(Registries.ITEM, Identifier.parse("agritech:tree_soils"))))),
            List.of(
                new com.misterd.agritech.recipe.DropEntry(items.getOrThrow(ResourceKey.create(Registries.ITEM, Identifier.parse(block.getTreeType().log()))).value(), 2, 4, 1f),
                new com.misterd.agritech.recipe.DropEntry(block.getResourcesType().leafFragmentItem().get(), 8, 20, block.getResourcesType().leafDropChance()),
                new com.misterd.agritech.recipe.DropEntry(block.getResourcesType().leafFragmentItem().get(), 4, 10, block.getResourcesType().leafDropChance() * 0.5f),
                new com.misterd.agritech.recipe.DropEntry(block.getResourcesType().saplingBlock(block.getTreeType().name()).get().asItem(), 1, 3, block.getResourcesType().saplingDropChance())
            )
        );
